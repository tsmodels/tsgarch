...
Type cgarchfun(objective_function<Type>* obj) {
    ...
    Type permanent_component_initialization = target_omega/(Type(1.0) - rho);
    if (cmodel(4) > 0.5) permanent_component_initialization = exp(permanent_component_initialization);
    // initialize variance
    for(j = 0;j<cmodel(0);j++) {
        // Set to the long run intercept \omega/(1-\rho)
        permanent_component(j) = permanent_component_initialization;
        sigma_squared(j) += initial_variance + permanent_component(j);
