...
Choosing wrongly is not symmetric in its consequences. Because the
`arma_errors` form cannot propagate a regressor effect at all, fitting
it to data generated by the `armax` form forces the observed
persistence to be explained through the error process instead, which
inflates $\phi$ while leaving $\tau$ close to its true value. In a
simulation with an AR(0.9) regressor and a true $\phi_1 = 0.6$, the
misspecified fit returned $\tau$ of 1.38 against a true 1.5, but an
autoregressive coefficient of 0.95, so a reader looking only at the
regressor coefficient would see nothing amiss while the persistence, the
half life and the rate at which mean forecasts revert were all badly
distorted. This is a single illustrative experiment in one direction rather
than a general result, but it is a reason to make the choice deliberately.
