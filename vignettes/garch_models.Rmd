...
-   does not implement joint ARFIMA-GARCH estimation, but does allow for a
    jointly estimated ARMA($p,q$) mean equation (see
...
Section \ref{sec:estimation:flavors} except `igarch` and `ewma`
(which share the plain `garch` mean equation and TMB template).
...
\ref{sec:estimation:arma}) for every flavor below except `igarch` and
`ewma`, with $\varepsilon_t$ unchanged in its role as the innovation
