...
-   does not implement joint ARFIMA-GARCH estimation, but does allow for a
    jointly estimated ARMA($p,q$)-X mean equation (see
    Section \ref{sec:estimation:arma}) in addition to the constant. With so
    many options for modelling the conditional mean beyond low order ARMA
    dynamics, many of which are available in the **tsmodels** framework, it
    was decided to keep this package's mean equation support deliberately
    limited rather than pursue full joint estimation of arbitrary mean
    dynamics with the variance equation. For a fully general mean equation
    (e.g. ARFIMA-X, or higher order fractional dynamics), the 2 step
    estimation approach, whereby the residuals of the conditional mean are
    passed to the variance dynamics estimation, remains available and may be
    more flexible in what can be achieved, even if potentially less
    efficient for small sized datasets. Additionally, the ARCH-in-mean model
    is no longer available, as it was found to have very limited value
    within the **tsmodels** framework or in general, this author's
    experience. The sibling
    [tsarma](https://github.com/tsmodels/tsarma) package remains available
    for standalone ARMA(p,q)-X modelling outside the GARCH setting.
...
\subsection{ARMA Mean Equation}\label{sec:estimation:arma}

By default $\mu_t$ reduces to either zero or a constant $\mu$
(controlled by the `constant` argument to `garch_modelspec`), in
which case $y_t - \mu_t = y_t - \mu$ is simply passed to the conditional
variance equation. Setting the `arma = c(p,q)` argument instead
specifies a jointly estimated ARMA($p,q$) mean equation:
...
and $\varepsilon_t$ (rather than $y_t - \mu$) is passed on to the
conditional variance equation of whichever GARCH flavor was chosen. Setting
`arma = c(0,0)` (the default) reduces Equation \ref{eq:arma_mean}
exactly to the pre-existing constant-mean behavior. The `constant`
argument remains independent of `arma`: it controls only whether $\mu$
is estimated or fixed at zero.

Regressors $x_{k,t}$ ($k = 1,\ldots,s$) can additionally enter the
conditional mean, via the `xreg` argument to `garch_modelspec`, with
coefficients $\tau_k$ (named `tau1`, `tau2`, ... in the `parmatrix`). The
`xreg_type` argument selects between two conventions. The default,
`xreg_type = "arma_errors"`, applies the ARMA recursion to the
...
\label{eq:armax_errors}
\end{equation}

This is the convention of `stats::arima(xreg = )` and of the sibling
**tsarma** package: the regressors are filtered by the AR polynomial,
$\tau_k$ is the long-run marginal effect of $x_{k,t}$ on $y_t$, and
$E\left[y_t\right] = \mu + \sum_k \tau_k E\left[x_{k,t}\right]$, so
$\mu$ retains its interpretation as the unconditional mean of $y$ net
of the regressor contribution. The alternative, `xreg_type = "armax"`,
is the **rugarch** convention:
...
Here the regressors are not filtered by the AR polynomial, so $\tau_k$
is the impact (contemporaneous) effect and the long-run effect of a
sustained change in $x_{k,t}$ is
$\tau_k/\left(1 - \sum_i \phi_i\right)$. The two conventions differ
only by the lagged-regressor term $\sum_k\tau_k x_{k,t-i}$ inside the
AR feedback, and are therefore algebraically identical whenever
$p = 0$; the choice of `xreg_type` only matters when the mean equation
has AR dynamics. Note that **tsarma** writes these coefficients
$\xi_k$; that symbol is already used in this package for the variance
equation regressors (see the "External Regressors" equations of each
flavor below), hence $\tau_k$ here. In both conventions it is again
$\varepsilon_t$ - not $y_t - \mu$ - that is passed on to the
conditional variance equation, exactly as in the pure ARMA case.

Because the recursion consumes $x_{k,t}$ at every time step,
forecasting and simulation require future regressor values
(`newxreg` in `predict()`, `xreg` in `simulate()`), and `tsfilter()`
likewise requires `newxreg` covering the appended observations. When
the model includes `xreg` but the future values are not supplied, a
zero matrix is substituted with a warning.

\paragraph{Choosing between the two conventions}

Expanding both conventions for an AR(1) mean equation with a single
regressor makes the difference precise. Writing the `arma_errors` form
out in levels:

\begin{equation}
y_t = \mu\left(1-\phi_1\right) + \phi_1 y_{t-1} + \tau_1 x_{1,t} - \phi_1\tau_1 x_{1,t-1} + \varepsilon_t
\label{eq:armax_adl_errors}
\end{equation}

and the `armax` form:

\begin{equation}
y_t = \mu\left(1-\phi_1\right) + \phi_1 y_{t-1} + \tau_1 x_{1,t} + \varepsilon_t
\label{eq:armax_adl}
\end{equation}

Both are restrictions of the same autoregressive distributed lag model
$y_t = c + \phi_1 y_{t-1} + \beta_0 x_{1,t} + \beta_1 x_{1,t-1} + \varepsilon_t$:
`arma_errors` imposes the common factor restriction
$\beta_1 = -\phi_1\beta_0$, whereas `armax` imposes $\beta_1 = 0$.
Neither is nested in the other, so the choice is a genuine modelling
decision rather than a reparameterization.

The practical consequence is clearest in how a shock to the regressor
propagates. Under `arma_errors`, since
$y_t = \mu + \sum_k\tau_k x_{k,t} + w_t$ with $w_t$ an ARMA process which
does not involve $x$, a one period unit blip in $x_{k,t}$ moves $y_t$ by
$\tau_k$ and leaves every subsequent observation untouched: the regressor
effect is static and the ARMA dynamics belong entirely to the error. Under
`armax` the regressor enters the same equation as the autoregressive
feedback, so the same blip moves $y_t$ by $\tau_k$, $y_{t+1}$ by
$\phi_1\tau_k$, $y_{t+2}$ by $\phi_1^2\tau_k$ and so on, cumulating to
$\tau_k/\left(1-\sum_i\phi_i\right)$. For a sustained unit increase in the
regressor, the long run shift in $y$ is $\tau_k$ under `arma_errors`
and $\tau_k/\left(1-\sum_i\phi_i\right)$ under `armax`.

This suggests, rather than dictates, where each is appropriate. An event or
calendar dummy usually has no reason to echo into subsequent periods at the
same rate as the persistence of the series itself, which is what
`armax` imposes, so the static reading of $\tau_k$ as the entire effect
is the easier one to defend. A covariate whose influence genuinely
accumulates and decays with the series, such as a policy rate or a sentiment
index, fits the `armax` propagation more naturally. When the AR order
is zero the two coincide exactly, and when the regressor is close to
serially uncorrelated the distinction is weak; it is with a persistent
regressor and a persistent mean equation that the two part company.

Choosing wrongly is costly, and the cost does not fall where intuition might
suggest. In a Monte Carlo experiment with 250 replications per cell
($n = 1500$, $\mu = 0.5$, $\phi_1 = 0.6$, $\tau_1 = 1.5$, Gaussian errors of
constant variance), fitting both conventions to data generated by each, for
an independent and for an AR(0.9) regressor, the average biases are:

\begin{center}
\begin{tabular}{llrr}
\hline
generated by & regressor & bias in $\tau_1$ & bias in $\phi_1$ \\
\hline
\texttt{arma\_errors} & independent & $-0.003$ & $-0.355$ \\
\texttt{arma\_errors} & AR(0.9)     & $-0.329$ & $-0.355$ \\
\texttt{armax}        & independent & $-0.420$ & $+0.088$ \\
\texttt{armax}        & AR(0.9)     & $-0.074$ & $+0.356$ \\
\hline
\end{tabular}
\end{center}

each row reporting the fit under the \emph{other} convention; the correctly
specified fit is unbiased to within 0.005 in every cell.

Two things follow. Whichever convention is the wrong one, the autoregressive
coefficient is the parameter most reliably damaged: the bias in $\phi_1$ is
roughly $\pm 0.36$ on a true value of 0.6 in three of the four misspecified
cells, which is easily enough to change the implied half life and the rate
at which mean forecasts revert by a factor of several. And how the damage
divides between $\tau_1$ and $\phi_1$ turns on the persistence of the
regressor rather than on the direction of the error: with an independent
regressor and `armax` data, the `arma_errors` fit misses $\tau_1$ by $-0.42$
while recovering $\phi_1$ tolerably, whereas with a persistent regressor the
same misspecification leaves $\tau_1$ nearly right and puts the error into
$\phi_1$ instead. Inspecting only the regressor coefficient can therefore be
reassuring and wrong at the same time.

The data are, however, decisive about which convention applies. Across all
1000 comparisons in that experiment the correctly specified convention had
the higher likelihood every single time, with mean gaps of 190 to 632 log
points, so the choice need not rest on judgement alone: estimate both and
compare.

The restriction is also testable. Since `armax` with both $x_{k,t}$ and
$x_{k,t-1}$ supplied as separate columns of `xreg` spans the
unrestricted distributed lag model, estimating that model and comparing
$\hat\beta_1$ against $-\hat\phi_1\hat\beta_0$ and against zero indicates
which of the two restrictions the data prefer.

Stationarity of the AR polynomial and invertibility of the MA polynomial
are required for $\phi$ and $\theta$ to be well defined, but rather than
impose these as nonlinear constraints on $\phi_i$ and $\theta_j$ directly
(which would require either finite-difference Jacobians or an
autodiff-aware eigendecomposition of a companion matrix, e.g. via
[RTMB](https://CRAN.R-project.org/package=RTMB)), the raw parameters
optimized over are instead a Durbin-Levinson / Jones partial
autocorrelation (PACF) reparameterization $r^{ar}_i, r^{ma}_j \in (-1,1)$,
from which $\phi$ (guaranteed to lie in the stationarity region) and
$\theta$ (guaranteed to lie in the invertibility region, via the
well-known AR/MA duality) are recovered by a smooth, exact-autodiff-
compatible recursion. This means only simple box constraints on
$r^{ar}_i,r^{ma}_j$ are required during optimization, with exact
(rather than finite-difference) Jacobians of the mean equation available
throughout via **TMB**. The raw PACF parameters (`arpacf`/`mapacf`
in the model's `parmatrix`) are not directly interpretable; the
transformed $\phi$/$\theta$ coefficients are what `summary()` displays,
and are also available via `arma_coefficients()`.

When `arma` is active, `fitted()` returns the (now genuinely
time-varying) conditional mean $\mu_t = \mu + \sum_i\phi_i\left(y_{t-i}-\mu\right) + \sum_j\theta_j\varepsilon_{t-j}$
rather than the constant $\mu$, and `residuals()` returns the true
ARMA innovations $\varepsilon_t = y_t - \mu_t$ rather than $y_t - \mu$.
Both `predict()` and `simulate()` account for the ARMA dynamics
in the mean forecast/simulated paths, and `tsfilter()` continues the
ARMA recursion when incrementally filtering newly appended observations.
The combined pre-sample/burn-in length used internally is
$\max\left(\text{GARCH order}, \text{ARMA order}\right)$, so that e.g. an
ARMA(2,2) mean equation with a GARCH(1,1) variance equation initializes
correctly. ARMA support is available for all GARCH flavors described in
Section \ref{sec:estimation:flavors}, including `igarch` and `ewma`.

An entire AR and/or MA polynomial can instead be fixed at target
coefficients rather than estimated, by setting `value` to the desired
ar/ma coefficients directly (not $r^{ar}_i$/$r^{ma}_j$) and `estimate = 0`
on every `arpacf`/`mapacf` row of the relevant polynomial in the spec's
`parmatrix`; the corresponding raw pacf values are computed automatically.
Fixing only some (not all) of the lags of a given polynomial is not
supported - since $\phi_i$ depends jointly on $r^{ar}_1,\ldots,r^{ar}_p$
(and similarly for $\theta_j$), there is no way to hold one lag's
coefficient fixed while leaving others free without either giving up the
stationarity/invertibility guarantee or reintroducing a nonlinear
constraint - and raises an error, as does supplying a fixed target that is
not itself stationary (AR) or invertible (MA).
...
This is the mean equation used by default (`arma = c(0,0)`); when an
ARMA($p,q$) mean equation is instead requested, $\mu_t$ becomes the
time-varying conditional mean of Equation \ref{eq:arma_mean} (Section
\ref{sec:estimation:arma}) for every flavor below, with $\varepsilon_t$ unchanged in its role as the innovation
driving each flavor's conditional variance equation.
...
Additionally, `arma_coefficients` returns the estimated AR/MA coefficients
(Section \ref{sec:estimation:arma}) implied by the raw partial
autocorrelation parameters used internally during estimation, for models
specified with a non-trivial `arma` order.
...
includes a number of tests which were previously part of **rugarch** is
already on CRAN.
